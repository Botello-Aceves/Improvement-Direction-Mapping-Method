...
diff(['mop = ',testname,'(mop,',num2str(dimension),');']);
end
...
%% SCH function generator
function p = schaffer(p, dim)
 p.name = 'SCH';
 p.od = 2;
 p.pd = 1;
 p.domain =  [-10 10];
...
%% SPH function generator
function p = sphere(p, dim)
 p.name = 'SPH';
 p.od = 2;
 p.pd = dim;
 p.domain = 3*[-ones(dim, 1) ones(dim, 1)];
 p.func = @evaluate;
 p.diff = @jacobian;
 
    %SPH evaluation function.
    function y = evaluate(x)
      y = zeros(2, 1);
      y(1)= x(:)'*x(:);
      y(2)= (x(:) - 2)'*(x(:) - 2);
...
%% SCH2 function generator
function p = schaffer2(p, dim)
 p.name = 'SCH2';
 p.od = 2;
 p.pd = 1;
 p.domain =  [-5 10];
...
%% FON function generator
function p = fonseca(p, dim)
 p.name = 'FON';
 p.od = 2;
 p.pd = dim;
 ...
 p.diff = @jacobian;
    ...
    %FON evaluation function.
    function y = evaluate(x)
      y = zeros(2,1);
      n = 1.0/sqrt(length(x));
      y(1)= 1.0 - exp(-sum((x - n).^2));
      y(2)= 1.0 - exp(-sum((x + n).^2));
    end
    ...
    function dy = jacobian(x)
      ...
      n = 1.0/sqrt(length(x));
      for i = 1:length(x)
          dy(1, i) = (x(i) - n)*exp(-sum((x - n).^2));
          dy(2, i) = (x(i) + n)*exp(-sum((x + n).^2));
      end
    end
end

%% KUR function generator
function p = kursawe(p, dim)
 p.name = 'KUR';
 p.od = 2;
 p.pd = 3;
 p.domain =  [-5*ones(3,1) 5*ones(3,1)];
 p.func = @evaluate;
 p.diff = @jacobian;
 
    %KUR evaluation function.
    function y = evaluate(x)
      y = zeros(2,1);
      n = exp(-0.2*sqrt(x(1)^2+x(2)^2)) + exp(-0.2*sqrt(x(2)^2+x(3)^2));
      m = abs(x(1))^(0.8) + abs(x(2))^(0.8) + abs(x(3))^(0.8);
      k = sin(x(1)^3) + sin(x(2)^3) + sin(x(3)^3);
      y(1)= -10*n;
      y(2)= m + 5*k;
    end
    ...
    function dy = jacobian(x)
      dy = zeros(2, 3);
      dy(1, 1) = 4*x(1)*exp(0.2*(x(1)^2 + x(2)^2)^(1/2))*(x(1)^2 + x(2)^2)^(-1/2);
      dy(1, 1) = 4*x(2)*(exp(0.2*(x(1)^2 + x(2)^2)^(1/2))*(x(1)^2 + x(2)^2)^(-1/2) + exp(0.2*(x(2)^2 + x(3)^2)^(1/2))*(x(2)^2 + x(3)^2)^(-1/2));
      dy(1, 3) = 4*x(3)*exp(0.2*(x(2)^2 + x(3)^2)^(1/2))*(x(2)^2 + x(3)^2)^(-1/2);
      dy(2, 1) = 15*x(1)^2*cos(x(1)^3) + (4*sign(x(1)))/(5*abs(x(1))^(1/5));
      dy(2, 2) = 15*x(2)^2*cos(x(2)^3) + (4*sign(x(2)))/(5*abs(x(2))^(1/5));
      dy(2, 3) = 15*x(3)^2*cos(x(3)^3) + (4*sign(x(3)))/(5*abs(x(3))^(1/5));
    end
end

%% POL function generator
function p = poloni(p, dim)
 p.name = 'POL';
 p.od = 2;
 p.pd = 2;
 p.domain =  [-pi*ones(2,1) pi*ones(2,1)];
 p.func = @evaluate;
 p.diff = @jacobian;
 
    %POL evaluation function.
    function y = evaluate(x)
      y = zeros(2,1);
    ...
    function dy = jacobian(x)
      ...
      B1 = 0.5*sin(x(1)) - 2*cos(x(1)) +   sin(x(2)) - 1.5*cos(x(2));
	  B2 = 1.5*sin(x(1)) -   cos(x(1)) + 2*sin(x(2)) - 0.5*cos(x(2));
      dB11 = 0.5*cos(x(1)) + 2*sin(x(1));
      dB12 = cos(x(2))   + 1.5*sin(x(2));
      dB21 = 1.5*cos(x(1)) +   sin(x(1));
      dB22 = 2*cos(x(2)) + 0.5*sin(x(2));
      dy(1, 1) = 1 - 2*(A1 - B1)*dB11 - 2*(A2 - B2)*dB21;
      dy(1, 2) = 1 - 2*(A1 - B1)*dB12 - 2*(A2 - B2)*dB22;
      dy(2, 1) = 2*(x(1) + 3);
      dy(2, 2) = 2*(x(2) + 1);
    end
end

%% VNT function generator
function p =  viennet(p, dim)
 p.name = 'VNT';
 p.od = 3;
 p.pd = 2;
 p.domain =  [-2*ones(2,1) 2*ones(2,1)];
 p.func = @evaluate;
 p.diff = @jacobian;
 
    %VNT evaluation function.
    function y = evaluate(x)
      y = zeros(3,1);
      y(1)= x(1)^2 + (x(2) - 1)^2;
      y(2)= x(1)^2 + (x(2) + 1)^2 + 1;
...
%% VNT2 function generator
function p =  viennet2(p, dim)
 p.name = 'VNT2';
 p.od = 3;
 p.pd = 2;
 p.domain =  [-4*ones(2,1) 4*ones(2,1)];
 p.func = @evaluate;
 p.diff = @jacobian;
 
    %VNT2 evaluation function.
    function y = evaluate(x)
      y = zeros(3,1);
      y(1)= (x(1) - 2)^2/2 + (x(2) + 1)^2/13 + 3;
      y(2)= (x(1) + 2 - 3)^2/36 + (2*x(2) - 1)^2/8 - 17;
      y(3)= (1 + 2*x(2) - 1)^2/175 + (2*x(2)^2 - x(1))^2/17 - 13; 
    end
...
%% VNT3 function generator
function p =  viennet3(p, dim)
 p.name = 'VNT3';
 p.od = 3;
 p.pd = 2;
 p.domain =  [-3*ones(2,1) 3*ones(2,1)];
 p.func = @evaluate;
 p.diff = @jacobian;
 
    %VNT3 evaluation function.
    function y = evaluate(x)
      y = zeros(3,1);
      y(1)= (x(1)^2 + x(2)^2)/2 + sin(x(1)^2 + x(2)^2);
...
function MatOut = repmat(MatIn,REPN)
